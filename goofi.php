...
if (!empty($_GET['family'])){
	...
	$family=str_replace(array("\0",'%20'), array('','+'),rawurlencode(strip_tags($_GET['family'])));
	$css_filename=$family.'.css';
	$css_file_url='http://fonts.googleapis.com/css?family='.$family;
	if (!is_file($css_filename)){
		...
		foreach($urls['url'] as $nb=>$url){
			$font_file=str_replace(array("\0",'%20'), array('','+'),rawurlencode($urls['name'][$nb])).basename($url);
